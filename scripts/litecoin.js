...
const litecoin = () => {
	...
	(async function() {
		...
		let coinbasePrice = Object.entries(await exchange.fetchTicker('LTC/USD'))[5][1];
		console.log(chalk.green.inverse(`LTC price in Coinbase is $${coinbasePrice}`));
		...
		let binancePrice = Object.entries(await exchange2.fetchTicker('LTC/USDT'))[5][1];
		console.log(chalk.green.inverse(`LTC price in Binance is $${binancePrice}`));
		...
		let bittrexPrice = Object.entries(await exchange3.fetchTicker('LTC/USD'))[5][1];
		console.log(chalk.green.inverse(`LTC price in Bittrex is $${bittrexPrice}`));
		...
		let poloniexPrice = Object.entries(await exchange4.fetchTicker('LTC/USDT'))[5][1];
		console.log(chalk.green.inverse(`LTC price in Poloniex is $${poloniexPrice}`));
		...
		let krakenPrice = Object.entries(await exchange5.fetchTicker('LTC/USD'))[5][1];
		console.log(chalk.green.inverse(`LTC price in Kraken is $${krakenPrice}`));
