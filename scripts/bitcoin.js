...
const bitcoin = () => {
	...
	(async function() {
		...
		let coinbasePrice = Object.entries(await exchange.fetchTicker('BTC/USD'))[5][1];
		console.log(chalk.green.inverse(`BTC price in Coinbase is $${coinbasePrice}`));
		...
		let binancePrice = Object.entries(await exchange2.fetchTicker('BTC/USDT'))[5][1];
		console.log(chalk.green.inverse(`BTC price in Binance is $${binancePrice}`));
		...
		let bittrexPrice = Object.entries(await exchange3.fetchTicker('BTC/USD'))[5][1];
		console.log(chalk.green.inverse(`BTC price in Bittrex is $${bittrexPrice}`));
		...
		let poloniexPrice = Object.entries(await exchange4.fetchTicker('BTC/USDT'))[5][1];
		console.log(chalk.green.inverse(`BTC price in Poloniex is $${poloniexPrice}`));
		...
		let krakenPrice = Object.entries(await exchange5.fetchTicker('BTC/USD'))[5][1];
		console.log(chalk.green.inverse(`BTC price in Kraken is $${krakenPrice}`));
